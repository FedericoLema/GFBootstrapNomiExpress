...
Public Class FRMgen_proy
    Public cndb As MySqlConnection
    Public appath As String

    Private Sub FRMgen_proy_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        'Dim Sql As String
        'Dim i As Integer
        ''DateTimePicker1.Format = DateTimePickerFormat.Short ' = "short"
        'Dim cndb As New MySqlConnection("server=localhost; userid=root; database=db_nomina")
    ...
    Private Sub carga_tablas()
        ...
        Sql = "SELECT DISTINCT(tabla) FROM tablas"
        'Sql = "SELECT COLUMNS.* FROM COLUMNS where TABLE_SCHEMA = 'erp_lite' and TABLE_NAME = 'empresas'"
        'Sql = Sql & " ORDER BY TABLA_NAME"
        'MsgBox(Sql)
        Dim cmd As New MySqlCommand(Sql, cndb)
        ...
        For i = 0 To dt.Rows.Count - 1
            CargaCampos(dt.Rows(i)(0).ToString)
            NombreFormatexbox.Text = "FRM" & dt.Rows(i)(0).ToString
            TituloFormatexbox.Text = Replace(Mid(dt.Rows(i)(0).ToString, 1, 1).ToUpper & Mid(dt.Rows(i)(0).ToString, 2, Len(dt.Rows(i)(0).ToString)), "_", " ")
            GeneraTabla(dt.Rows(i)(0).ToString)
        Next i
    ...
    Private Function CargaCampos(NombreTabla As String)
        ...
        Dim cndb As New MySqlConnection("server=127.0.0.1; port=3306; userid=rednel; password=expres2021*; database=erp; SslMode=none")
        Dim dt As New DataTable()
        Sql = "SELECT * FROM tablas where tabla = '" & NombreTabla & "' order by tabla,orden"
    ...
    Private Sub GeneraTabla(nombretabla As String) 'sender As Object, e As EventArgs) Handles BTNgenerar.Click
        ...
        objStreamWriter = New StreamWriter(appath & "\Formas\templates\frm_" & nombretabla & "_list.html")
        ...
        objStreamWriter = New StreamWriter(appath & "\Formas\templates\frm_" & nombretabla & "_show.html")
        ...
        objStreamWriter = New StreamWriter(appath & "\Formas\frm_" & nombretabla & ".py")
